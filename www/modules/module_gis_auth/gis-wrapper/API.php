...
class API {
     ...
     * @param int $max
     */
    public function garbageCollection($max = 5) {
        $a = $b = $max;
        while(count($this->_dynamicAPIs) > $a) {
            $name = array_shift($this->_dynamicAPIs);
            if(in_array($name, $this->_dynamicKeep)) {
                $a++;
                array_push($this->_dynamicAPIs, $name);
            } else {
     ...
     * __clone()
     *
     * gets calles when an API object is cloned. This is normally the case, when we retrieve a new instance of a dynamic API. Therefore all the instances of dynamic apis and endpoints are deleted as well as the static apis and endpoints are cloned.
     */
    function __clone() {
        $tmp = $this->_apis;
        $this->_apis = array();
        foreach($tmp as $name => $api) {
            if($name == ($api instanceof API) ? $api->getName() : $api->name) {
        ...
        foreach($tmp as $name => $ep) {
            if($name == ($ep instanceof Endpoint) ? $ep->getName() : $ep->name) {
